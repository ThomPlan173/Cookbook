...
/*#----------------------------------------------ALEXANDRE___DEBUT------------------------------------------------------------*/
//page permettent de verifier le formulaire de l'ajout d'une recette et l'ajoute si il n'y a pas eu de problème et upload le fichier image
...
$response = $ad->verifRecette($_SESSION['nom'],$_SESSION['description'],$_SESSION['preparation']); // verifie si le formulaire d'ajout de recette ne présente pas d'erreurs
...
if($response['granted']){ // si il n'y a pas d'erreur (hormis l'upload de fichiers), on verifie si il n'y a pas d'erreurs de quantitées
    ...
    if($upload!=""): // si il n'y a pas d'erreur d'upload, ajoute la recette a la base de données, ajoute les tags et les ingrédients cochées avec les quantitées, unitées
        ...
        foreach ($recettes as $r){ // récupère la dernière recette ( celle ajouté )
            $idRecette = $r->idRecette;
        }
        foreach ($tagsChecked as $tc){ // ajoute les tags à la recette
            $cb->addTagRecette($tc->idTag, $idRecette);
        }
        $i = 0;
        foreach ($ingrsChecked as $igc){// ajoute les ingrédients à la recette avec quantitées , unitées
...
header("Location: ".$_SERVER['HTTP_REFERER']); // renvoie vers add
exit();
